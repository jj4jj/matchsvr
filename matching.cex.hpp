...
//File generated time: Mon Jun 13 19:46:40 2016
...
struct MatchingQueueMatcher_ST : public serializable_t<MatchingQueueMatcher_ST, MatchingQueueMatcher> {
    ...
    int     compare(const MatchingQueueMatcher_ST & rhs_) const {
        ...
        if (this->team_id < rhs_.team_id){
            return -1;
        }
        else if(this->team_id > rhs_.team_id){
            return 1;
        }
        return cmp;
    }
    ...
    size_t hash() const {
        size_t avhs[] = {
            (size_t)(this->team_elo),
            (size_t)(this->team_id),
        };
        return pbdcex::hash_code_merge_multi_value(avhs, 2);
    }
};
struct MatchingWaitigTeam_ST : public serializable_t<MatchingWaitigTeam_ST, MatchingWaitigTeam> {
    ...
    int     convto(MatchingWaitigTeam & tomsg_) const {
        int ret = 0;
        tomsg_.Clear();
        for (size_t i = 0;i < teams.count; ++i){
    ...
    int     convfrom(const MatchingWaitigTeam & frommsg_) {
		int ret = 0;
        this->teams.count=0;
        for (size_t i = 0; i < (size_t)frommsg_.teams_size() && i < MACTCHING_BUCKET_MAX_WAITING_TEAM_NUM; ++i, ++(this->teams.count)){
...
struct MatchedResult_ST : public serializable_t<MatchedResult_ST, MatchedResult> {
    array_t<uint64_t, MATCHED_RESULT_TEAM_NUM>       teams;
    ////////////////////////////////////////////////////////////////////////
    void    construct(){
        if(sizeof(*this) < 1024){
            memset(this, 0, sizeof(*this));
    ...
    int     convfrom(const MatchedResult & frommsg_) {
		int ret = 0;
        this->teams.count=0;
        for (size_t i = 0; i < (size_t)frommsg_.teams_size() && i < MATCHED_RESULT_TEAM_NUM; ++i, ++(this->teams.count)){
...
struct MatchingBucket_ST : public serializable_t<MatchingBucket_ST, MatchingBucket> {
    ...
    int     convto(MatchingBucket & tomsg_) const {
        int ret = 0;
        tomsg_.Clear();
        for (size_t i = 0;i < waitings.count; ++i){
            ret = this->waitings.list[i].convto(*tomsg_.add_waitings());
            if (ret){return __LINE__+ret;}
            }
        for (size_t i = 0;i < matchers.count; ++i){
    ...
    int     convfrom(const MatchingBucket & frommsg_) {
        ...
        for (size_t i = 0; i < (size_t)frommsg_.waitings_size() && i < MATCHING_TEAM_MAX_WAITING_MERGING_MEMBER_NUM; ++i, ++(this->waitings.count)){
            ret = this->waitings.list[i].convfrom(frommsg_.waitings(i));
            if (ret) {return __LINE__+ret;}
        }
        this->matchers.count=0;
        for (size_t i = 0; i < (size_t)frommsg_.matchers_size() && i < MATCHING_TEAM_MAX_NUM_IN_LEVEL; ++i, ++(this->matchers.count)){
...
struct MatchingTeam_ST : public serializable_t<MatchingTeam_ST, MatchingTeam> {
    ...
    int     convfrom(const MatchingTeam & frommsg_) {
        ...
        for (size_t i = 0; i < (size_t)frommsg_.members_size() && i < MATCHING_TEAM_MAX_MEMBER_NUM; ++i, ++(this->members.count)){
    ...
    size_t hash() const {
        size_t avhs[] = {
             this->members.hash(),
            (size_t)(this->join_time),
            this->point.hash(),
        };
        return pbdcex::hash_code_merge_multi_value(avhs, 3);
    }
};
struct MatchingQueue_ST : public serializable_t<MatchingQueue_ST, MatchingQueue> {
    ...
    int     convfrom(const MatchingQueue & frommsg_) {
        ...
        for (size_t i = 0; i < (size_t)frommsg_.buckets_size() && i < MACTCHING_QUEUE_MAX_LEVEL_NUM; ++i, ++(this->buckets.count)){
        ...
        for (size_t i = 0; i < (size_t)frommsg_.results_size() && i < MACTCHING_QUEUE_MAX_MACTCHED_RESULT_NUM; ++i, ++(this->results.count)){
    ...
    size_t hash() const {
        size_t avhs[] = {
            (size_t)(this->team_member_num),
             this->buckets.hash(),
            (size_t)(this->cur_time),
            (size_t)(this->cur_ms_insec),
             this->results.hash(),
            (size_t)(this->fetched_results),
        };
        return pbdcex::hash_code_merge_multi_value(avhs, 6);
    }
};
